...
class Oauth
{
	...
	public function updateDatastore($token)
	{
		...
		if ($tokenValue)
		{
			...
			$tokenValue->setClientId($dn);
			$tokenValue->setMachinePersonId($buid);
			$this->em->persist($tokenValue);
			$this->em->flush($tokenValue);
		} else {
			$newToken = new SparkToken();
			$newToken->setSparkToken($token);
			$newToken->setClientId($dn);
			$newToken->setMachinePersonId($buid);
	...
	public function getBotToken()
	{
		...
		{
			return 'Bearer ' . $this->configuration['bottoken'];
		} else {
	...
	public function setMachineDetail($options = array())
	{
		...
		$pr        = $pclient->patch('https://identity.webex.com/organization/'. $this->configuration['machine_org'] .'/v1/Machines/' . $mid ,[
				'headers' => ['Authorization'=> $it, 'Content-Type' => 'application/json'],
				'body'    => $body,
				'verify'  => false
